...
The cat_avro utility prints 20 records and then the schema of the records. Note that a schema is included with each data file, so that it lives with the data. This is convenient. From now on we don't have to cast our data as we load it like we did before.
